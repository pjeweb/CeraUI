...
>
    <div>
        ...
        <Card.Description>{Object.keys(currentNetwoks).length} Networks, {getAvailableNetworks(currentNetwoks).length} Available, {getUsedNetworks(currentNetwoks).length} used, {totalBandwith} Kbps</Card.Description>
    </div>
    ...
    <div class="space-y-8">
        {#each Object.entries(currentNetwoks) as [name, network] }
            <div class="flex items-center">
                <Toggle variant="outline" class={cn(network.enabled ? 'data-[state=on]:bg-green-600' : 'bg-red-600')} title={network.enabled ? 'Disable network' : 'Enable Network'}
                ...
                e-y-1">
                    ...
                    <p class="text-muted-foreground text-sm">Identifier: {name}</p>
                </div>
