<script lang="ts">
    ...
    $inspect(hotspotProperties).with(
        console.log
    )
</script>

<SimpleAlertDialog confirmButtonText="Save" onOpenChange={(open)=> open && resetHotSpotProperties()}
    ...
    {#snippet description()}
        <div class="grid gap-2">
            <div class="grid gap-1">
                ...
                <Input bind:value={hotspotProperties.name}
                       id="name"
                       placeholder="CERABOX"
                       autocapitalize="none"
                       autocomplete="off"
            ...
            <div class="grid gap-1">
                <Label for="channel" class="ml-1 mb-2">Channel</Label>
                <Select.Root id="channel" onSelectedChange={(value)=>{
